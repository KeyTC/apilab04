...
routes.get('/',(req, res)=>{
    req.getConnection((err,connect)=>{
        ...
        connect.query('SELECT Id,concat(Proveedor," : ",TipoProveedor) AS Proveedor,Direccion,Provincia,Canton,Distrito,TipoCedula,Cedula,Contacto,Telefono,Correo FROM proveedores WHERE Estado = 1', (err,rows)=>{
            if (err) return res.send(err)
            return res.json({
                success: true,
                data: rows
